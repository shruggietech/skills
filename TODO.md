...
- [ ] **`shruggie-docs` runtime verification.** The v1 commit builds only the skill source; no sample `.docx` files were generated. Generate one sample of each document type (SOW, SOA, MSA, Internal Report, Invoice, Letter) under the Anthropic execution environment using `assets/document-template.js`, run `assets/embed-fonts.py` against each, validate with `/mnt/skills/public/docx/scripts/office/validate.py`, then open each in Microsoft Word and LibreOffice Writer on a host that has neither Geist nor Space Grotesk installed and confirm the typography, color, justification, footer behavior, and font embedding render correctly. Surface any failures back to the operator before refining the skill description.
